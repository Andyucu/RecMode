...
- **Icons: `Segoe Fluent Icons` font, not embedded SVGs.** RecMode renders the design's icon language via the Windows `Segoe Fluent Icons` (fallback `Segoe MDL2 Assets`) font glyphs — the mechanism the source tiles already use, so no SVG-geometry system is needed. **Done 2026-07-05:** nav sidebar icons (video/grid/calendar/gear) **and** per-card leading icons on every Settings card + the Schedule card (`CardIcon` style; brush/palette/video/page/speaker/sliders/folder/rename/stopwatch/mouse/pointer/keyboard/power/sync/calendar). Glyph choices verified by screenshot.
