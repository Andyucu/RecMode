...
internal static class CaptureInterop
{
    ...
    public static bool TryGetCaptureBounds(CaptureTarget target, out RegionRect bounds)
    {
        ...
        if (target.Kind == CaptureKind.Window && TryGetWindowRect(target.Handle, out bounds))
        {
        ...
        {
            RegionRect r = target.Region ?? new RegionRect(0, 0, monitor.Width, monitor.Height);
            bounds = new RegionRect(monitor.X + r.X, monitor.Y + r.Y, r.Width, r.Height);
            return bounds.Width > 0 && bounds.Height > 0;
        }
        bounds = default;
        return false;
